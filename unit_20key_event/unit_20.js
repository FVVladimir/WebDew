...
function t5(event) {
    ...
    document.querySelector('.out-5').textContent += symbol;
...
function t6(event) {   
    ...
    if (event.keyCode >= 97 && event.keyCode <= 122) {
        ...
        document.querySelector('.i-6').value = ourString;

        console.log(ourString, ourString.length)
...
function t7() {
    ...
    document.querySelector('.out-7').textContent = a7[num];
...
function t8(event) {
    ...
    let symb = event.key;                                      // 1. Получаем из event введенный символ
        if(a8[symb] !== undefined) symb = a8[symb]             // 2. Проверяем наличие такого ключа в a8 - уже делали это раньше!!!!
          document.querySelector('.out-8').textContent += symb;// 3. Если есть дописываем в out-8 символ из массива a8. Если нет - введенный символ.
}
...
function t9(event) {    
    ...
    document.querySelector('.out-9').textContent = arrowDown;
}
...
// Task 10 ============================================
/*  Дан input .i-10 и блок .block-10. Добавьте событие на input,
//  при нажатии клавиш стрелка вправо и стрелка влево увеличивать ширину блока.
...
function t10(event) {
    ...
    if(event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
        ...
        w += 1;
        document.querySelector('.block-10').style.wigth = w;
        
    } else if (event.key === 'ArrowDown' || event.key === 'ArrowUp'){
         
        h += 1;
        document.querySelector('.block-10').style.height = h;
