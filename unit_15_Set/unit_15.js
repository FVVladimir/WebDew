...
// которые пользователь вводит в i - 2. Функция должна выводить в консоль s2 после каждого добавления элемента.
...
// которую пользователь вводит в i - 3. Функция должна выводить в консоль s3 после каждого удаления элемента.
...
// При нажатии b-4 выполняете функцию f4. Функция должна проверить наличие в наборе s4 строки
// введенной пользователем в i - 4. Если строка есть - то вывести в out - 4 true.Если нет - false.
...
// а длина пароля больше(строго) 6 то выводите в out - 7 число 1.
...
const f7 = () => {
    ...
    // console.log(inp, inp.length, set7.size);
    if (inp.length === set7.size && inp.length > 6) {
        out.textContent = 1;
    } else {
        out.textContent = 0;
    }
...
// Функция должна перебрать набор s8 и добавить в массив ar8 только числа больше 5. Выведите массив в консоль.
...
const f9 = (our_set) => {
    ...
    // console.log(Array.from(s9).join(' '));
    // console.log([...s9].join(' '));
    ourStr = Array.from(our_set).join(' ');
    return ourStr;
}
...
// так, что каждая буква - отдельный элемент массива.Потом создать набор на основе массива и возвратить его.
...
const f13 = (str) => {
    ...
    let res = {};
    let str13_1 = str13.split('');
    let s13 = new Set(str13_1);

    for (let item of s13) {
        let count = 0;
        for (i = 0; i < str13_1.length; i++) {
            if (item === str13_1[i]) {
                count++;
            }
            res[item] = count;
